<script lang="ts">
  ...
  $: filesToDisplay =
    currentSearch.trim() && filteredList.length > 0 ? filteredList : allFilesList;
  ...
  const handleShare = (event: CustomEvent<{ file: any; url: string }>) => {
    ...
    files.updateFile(file.id, { shared: true, share_url: url });

    if (previewFile && previewFile.id === file.id) {
      previewFile = { ...previewFile, shared: true, share_url: url };
    }
  };
...
<section class="px-4 py-4 md:px-6 md:py-6" style="color: hsl(var(--foreground))">
  ...
  <div class="grid grid-cols-3 gap-3 mb-6">
    <div class="rounded-lg border p-3 text-center" style="border-color: hsl(var(--border)); background-color: hsl(var(--card));">
      <p class="text-xs font-medium mb-1" style="color: hsl(var(--muted-foreground))">Total Files</p>
    ...
    <div class="rounded-lg border p-3 text-center" style="border-color: hsl(var(--border)); background-color: hsl(var(--card));">
      <p class="text-xs font-medium mb-1" style="color: hsl(var(--muted-foreground))">Shared Files</p>
    ...
    <div class="rounded-lg border p-3 text-center" style="border-color: hsl(var(--border)); background-color: hsl(var(--card));">
      <p class="text-xs font-medium mb-1" style="color: hsl(var(--muted-foreground))">Storage Used</p>
      <p class="text-xl font-bold" style="color: hsl(var(--foreground))">{formatBytes(currentStats.totalSize)}</p>
      <div class="mt-1 flex justify-center">
    ...
    <div class="p-4">
      {#if $files?.isLoading}
        <div class="text-center py-6">
          ...
          <p class="text-sm mt-2" style="color: hsl(var(--muted-foreground))">Loading files...</p>
        </div>
      ...
      {:else}
        ...
        {#if filesToDisplay.length > 6}
          <div class="mt-4 pt-3 border-t text-center" style="border-color: hsl(var(--border))">
            <a
              href="/dashboard/files"
  ...
  <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
    <div class="flex items-center gap-1.5 p-2.5 rounded-md" style="background-color: hsl(var(--card)); border: 1px solid hsl(var(--border))">
